...
local DATA_TIMEOUT_OUTBOUND_SEC = 0.4
local DATA_TIMEOUT_INBOUND_SEC = 0.1
local CHECK_UNITS_PER_SEC = 1
local SEND_SERVER_INFO_SEC = 1
local DDOS_RATE_LIMITER = 2
...
function updateObjs(ourArgument, time)
    ...
    return time + DATA_TIMEOUT_OUTBOUND_SEC
end
...
function sendRequest(outObj)
    if outObj.unitId ~= nil then
        local timeNow = timer.getAbsTime()
        if udpEventRateLimiter[outObj.unitId] ~= nil then
            if udpEventRateLimiter[outObj.unitId] < timeNow then
                udpEventRateLimiter[outObj.unitId] = timeNow + DDOS_RATE_LIMITER
        ...
        if request.action == "processGCIDetectionByName" then
            ...
            if type(request.ewrNames) == 'table' then
                for nIndex = 1, #request.ewrNames do
                    ...
                    if curUnit ~= nil then
                        local getGroup = curUnit:getGroup()
                        if getGroup ~= nil then
                            local ewrController = getGroup:getController()
                            local detectedTargets = ewrController:getDetectedTargets(Controller.Detection.RADAR)
                            if type(detectedTargets) == 'table' then
...
function runPerFrame(ourArgument, time)
    ...
    return time + DATA_TIMEOUT_INBOUND_SEC
end
...
timer.scheduleFunction(runPerFrame, {}, timer.getTime() + DATA_TIMEOUT_INBOUND_SEC)
...
function clientEventHandler:onEvent(_event)
    local status, err = pcall(function(_event)
        if _event == nil or eventTypes[_event.id] == nil then
            ...
            -- this is to keep the UDP packet sending to a minimum, we need more UDP bandwidth
            local sendEvent = false
            if eventTypes[_event.id] == "S_EVENT_SHOT" then sendEvent = true end
            if eventTypes[_event.id] == "S_EVENT_TAKEOFF" then sendEvent = true end
            if eventTypes[_event.id] == "S_EVENT_LAND" then sendEvent = true end
            if eventTypes[_event.id] == "S_EVENT_DEAD" then sendEvent = true end
            if eventTypes[_event.id] == "S_EVENT_PILOT_DEAD" then sendEvent = true end
            if eventTypes[_event.id] == "S_EVENT_BIRTH" then sendEvent = true end
            if eventTypes[_event.id] == "S_EVENT_PLAYER_ENTER_UNIT" then sendEvent = true end
            if eventTypes[_event.id] == "S_EVENT_SHOOTING_START" then
                -- rate limit machine gun, 1 even per second
                if curEvent.initiator.unitId ~= nil then
                    if udpEventRateLimiter[curEvent.initiator.unitId] ~= nil then
                        local timeNow = timer.getAbsTime()
                        if udpEventRateLimiter[curEvent.initiator.unitId] < timeNow then
                            udpEventRateLimiter[curEvent.initiator.unitId] = timeNow + DDOS_RATE_LIMITER
                            sendEvent = true
                        end
                    else
                        udpEventRateLimiter[curEvent.initiator.unitId] = 0
                    end
                end
            end
            if eventTypes[_event.id] == "S_EVENT_KILL" then sendEvent = true end

...
export async function actionAliveIds(obj: any): Promise<any> {
    return new Promise((resolve, reject) => {
        dbModels.unitModel.find(obj).select({unitId: 1}).setOptions({ lean: true }).exec((err: any, unitIds: any) => {
            if (err) { reject(err); }
            resolve(unitIds);
        });
    });
...
export async function unitSetBakedUnitsDead(): Promise<void> {
    return new Promise((resolve, reject) => {
        dbModels.unitModel.updateMany(
            {name: /^~/},
            {$set: {dead: true}},
...
export async function unitClearAllUnitIds(): Promise<void> {
    return new Promise((resolve, reject) => {
        dbModels.unitModel.updateMany(
            {},
            {$set: {unitId: null}},

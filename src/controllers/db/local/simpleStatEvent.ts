...
export async function simpleStatEventActionsReadLastShotEvent(obj: any): Promise<typings.ISimpleStatEvents[]> {
    return new Promise((resolve, reject) => {
        dbModels.simpleStatEventModel.find(obj).sort( { createdAt: -1 } ).limit(1).exec((
            err: any,
            dbStatEvents: typings.ISimpleStatEvents[]
        ) => {
            if (err) { reject(err); }
            resolve(dbStatEvents);
        });
    });
}

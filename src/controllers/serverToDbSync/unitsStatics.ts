 ...
 */
    ...
    if (unit.length > 0) {
        ...
        if (curData && curData.type && (_.includes(curData.type, "CVN_72") || _.includes(curData.type, "LHA_Tarawa") ||
            _.includes(curData.type, "Forrestal"))) {
            // console.log("carrier update location: ", curUnitName, curData.lonLatLoc);
            await ddcsControllers.campaignAirfieldActionUpdate({_id: curUnitName, centerLoc: curData.lonLatLoc, isResync: true});
        }
        /*
        ...
        if (unitObj.action !== "D") {
            if (curData.name) {
                ...
                if (_.includes(curData.name, "AI|baseAWACS|")) {
                    console.log("AWACS Spawn: ", unitObj.data.name);
                }

                // if unit is AWACS spawn support escort
                if (unitObj.action === "C" && unitObj.data.groupId > 0 && (_.includes(curData.name, "AI|baseAWACS|"))) {
                    // console.log("Spawn AWACS Escort: ", unitObj);
                    // disabled escort fighters until we get the lag under control
                    // await ddcsControllers.spawnEscortFighters(unitObj.data.groupId);
                }
                ...
                if ((curData.playername !== "" && curData.groupId) || _.includes(curData.name, "DU|")) {

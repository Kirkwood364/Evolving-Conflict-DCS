...
export async function staticTemplate(staticObj: any, templateName?: string): Promise<string> {
    // console.log("templateObj: ", staticObj);
    const curTemplate = (templateName) ? templateName : "staticUnit" ;
    const spawnTemplate = await ddcsControllers.templateRead({_id: curTemplate});
    const compiled = _.template(spawnTemplate[0].template);
...
export function getRndFromSpawnCat(
    ...
    const curTimePeriod = engineCache.campaign.timePeriod;
    let findUnits;
    const cPUnits: any[] = [];
    let randomIndex;
    ...
    if (!_.isEmpty(useUnitType)) {
        ...
        findUnits = _.filter(engineCache.unitDictionary, (ud) =>
            ud.spawnCat.includes("samRadar") && ud.spawnCatSec.includes("modern") && ud.enabled);
        // console.log("FINDUNITS1: ", findUnits, {spawnCat: "samRadar", spawnCatSec: "modern", enabled: true});
    } else {
        findUnits = _.filter(engineCache.unitDictionary, (ud) =>
            ud.spawnCat.includes(spawnCat) && ud.enabled);
        // console.log("FINDUNITS2: ", {spawnCat, enabled: true});
    }

    if (findUnits && findUnits.length > 0) {
        for (const unit of findUnits) {
            // console.log("UNITSPAWN: ", unit);
            const curCountry =
                _.intersection(
                    unit.config[ddcsControllers.getEngineCache().config.timePeriod].country,
                    ddcsControllers.engineCache.campaign.countrySides[(side || 0)]
                );
    ...
    if (curUnit) {
        ...
        if (curUnits.length > 0) {
            for (const cUnit of curUnits) {
                const curTimePeriodSpawnCount = cUnit.config[ddcsControllers.getEngineCache().campaign.timePeriod].spawnCount;
                if (cUnit.launcher) {
...
export function spawnSupportVehiclesOnFarp(baseName: string, side: number): typing.IUnitSpawnMin[] {
    const engineCache = ddcsControllers.getEngineCache();
    const curBase = _.find(engineCache.bases, {_id: baseName});
    // console.log("basename: ", baseName, curBase);
    if (curBase) {
        ...
        for (const val of sptArray) {
            ...
            const curCountry = _.intersection(
                curSpawnTemplate.config[ddcsControllers.getEngineCache().campaign.timePeriod].country,
                ddcsControllers.engineCache.campaign.countrySides[(side || 0)]
            );
...
export async function spawnSupportBaseGrp(baseName: string, side: number, init: boolean): Promise<void> {
    const farpBases = _.filter(ddcsControllers.getEngineCache().bases, (baseObj) => {
        return ((_.includes(baseObj._id, "_MOB") && baseObj.side === side) ||
            _.includes(baseObj._id, "_FOB")) && _.split(baseObj._id, " #")[0] === baseName;
    });
    console.log("Spawn Support Groups: ", baseName, side);
    // console.log("SSB: ", farpBases, baseName, side, init);

    for (const farp of farpBases) {
        await spawnUnitGroup(spawnSupportVehiclesOnFarp(farp._id, side), init, baseName, side);
    }
}
...
export async function spawnBaseReinforcementGroup(side: number, baseName: string, init: boolean, forceSpawn?: boolean): Promise<number> {
    ...
    let curSpokeNum;
    let infoSpwn;
    const curBaseSpawnCats: any = engineCache.campaign.spwnLimitsPerTick;
    ...
    for (const spawnCats of Object.keys(curBaseSpawnCats)) {
        ...
        for (let i = 0; i < spawnTicks; i++) {
            ...
            if (curRndSpawn.length > 0) {
                ...
                const curCountry = _.intersection(
                    infoSpwn.config[ddcsControllers.getEngineCache().campaign.timePeriod].country,
                    ddcsControllers.engineCache.campaign.countrySides[(side || 0)]
                );
        ...
        if (spawnCats === "antiAir" && spawnTicks > 0 && engineCache.campaign.timePeriod === "1978ColdWar") {
            totalUnits += await spawnLayer2Reinforcements("antiAir", 2, spawnTicks, side, baseName, init);
        }

        if (spawnCats === "mobileAntiAir" && spawnTicks > 0 && engineCache.campaign.timePeriod === "modern") {
...
export async function spawnLayer2Reinforcements(
    ...
    for (let i = 0; i < curTickCnt; i++) {
        ...
        const curCountry = _.intersection(
            curRndSpawn[0].config[ddcsControllers.getEngineCache().campaign.timePeriod].country,
            ddcsControllers.engineCache.campaign.countrySides[(side || 0)]
        );
...
export async function spawnDefenseChopper(playerUnitObj: typing.IUnit, unitObj: typing.IUnit): Promise<void> {
    ...
    const friendlyBase = await ddcsControllers.campaignAirfieldActionGetClosestFriendlyBase({
...
export async function spawnAtkChopper(playerUnitObj: typing.IUnit, unitObj: typing.IUnit): Promise<void> {
    ...
    const enemyBase = await ddcsControllers.campaignAirfieldActionGetClosestEnemyBase({
        unitLonLatLoc: playerUnitObj.lonLatLoc,
        playerSide: playerUnitObj.coalition
    });
    const friendlyBase = await ddcsControllers.campaignAirfieldActionGetClosestFriendlyBase({
...
export async function spawnBomberPlane(playerUnitObj: typing.IUnit, bomberObj: any): Promise<void> {
    ...
    const closeBase = await ddcsControllers.campaignAirfieldActionGetClosestEnemyBase({
        unitLonLatLoc: playerUnitObj.lonLatLoc,
        playerSide: playerUnitObj.coalition
    });
...
export async function spawnTankerPlane(
    ...
    const closeBase = await ddcsControllers.campaignAirfieldActionGetClosestBase({unitLonLatLoc: playerLoc});
    // console.log("closeBase: ", closeBase);
    curGrpObj = {
        ..._.cloneDeep(curSpwnUnit),
        groupName: curTkrName + "#" + _.random(1000000, 9999999),
        country: ddcsControllers.countryId.indexOf(tankerObj.country),
        countryName: tankerObj.country,
        unitCategory: ddcsControllers.UNIT_CATEGORY.indexOf("AIRPLANE"),
        routeLocs: [
            remoteLoc,
            playerLoc
        ]
    ...
    await ddcsControllers.sendMesgToCoalition(
        curSide,
        "TANKERPLANEOUT",
        [tankerObj.type, playerUnitObj.hdg, closeBase._id, tankerObj.details],
        20
    );
}
...
export async function spawnSupportPlane(baseObj: typing.ICampaignAirfield, side: number): Promise<void> {
    const engineCache = ddcsControllers.getEngineCache();
    let curBaseName;
    let curUnitName;
    let curUnitSpawn;
    ...
    curBaseName = "AI|1010101|" + baseObj._id + "|LOGISTICS|";
    ...
    curUnitName = "AI|1010101|" + baseObj._id + "|LOGISTICS|";
    ...
    console.log("Cargo Plane Out: ", side, baseObj._id);
    await ddcsControllers.sendMesgToCoalition(
        side,
        "CARGOSUPPORTPLANEOUT",
        [randomDir, baseObj._id],
        20
    );
}
...
export async function spawnStaticBaseBuilding(
    staticObj: any,
    init: boolean,
    baseObj?: any,
    side?: number,
    staticType?: string,
    spawnOnServerFirst?: boolean
): Promise<void> {
    if (init) {
        const staticLookupLogiBuilding = await ddcsControllers.unitDictionaryActionsRead({_id: staticType});
        const curCountry =
            _.intersection(
                staticLookupLogiBuilding[0].config[ddcsControllers.getEngineCache().campaign.timePeriod].country,
                ddcsControllers.engineCache.campaign.countrySides[(side || 0)]
            );
        if (curCountry.length > 0) {
            const curStaticObj = {
                ...
                name: baseObj._id + " " + staticType,
                _id: baseObj._id + " " + staticType,
                hdg: _.random(0, 359),
                alt: 0,
                lonLatLoc: ddcsControllers.getRandomLatLonFromBase(baseObj._id, "buildingPoly"),
                isActive: true
            };
            // console.log("STATIC1: ", curStaticObj.name, curStaticObj.lonLatLoc);
            // console.log("SL: ", curStaticObj, spawnOnServerFirst);
            if (spawnOnServerFirst) {
                await ddcsControllers.sendUDPPacket("frontEnd", {
                    actionObj: {
                        action: "CMD",
        ...
        const staticLookupLogiBuilding = await ddcsControllers.unitDictionaryActionsRead({_id: staticObj.type});
        staticObj.shape_name = staticLookupLogiBuilding[0].shape_name;
        ...
        // console.log("STATIC3: ", staticObj, curCMD);
...
export async function spawnStaticBuilding(
    ...
    // console.log("spawnStaticBuilding: ", staticObj, " - ", side, " - ", staticType, " - ", spawnOnServerFirst);
    const staticLookupLogiBuilding = await ddcsControllers.unitDictionaryActionsRead({_id: staticType});
    const curCountry =
        _.intersection(
            staticLookupLogiBuilding[0].config[ddcsControllers.getEngineCache().campaign.timePeriod].country,
    ...
    if (curCountry.length > 0) {
        const curStaticObj = {
            ...
            hdg: _.random(0, 359),
            alt: 0,
            lonLatLoc: staticObj.lonLatLoc,
            isActive: true
        };
        // console.log("STATIC1: ", curStaticObj.name);
        if (spawnOnServerFirst) {
            await ddcsControllers.sendUDPPacket("frontEnd", {
                actionObj: {
                    action: "CMD",
...
export async function spawnUnitGroup(spawnArray: typing.IUnitSpawnMin[], init: boolean, baseName?: string, side?: number): Promise<void> {
    ...
    if (spawnArray.length > 0) {
        ...
        for (const curUnit of spawnArray) {
            ...
            // should be removed, must test when removed!!!
...
export async function spawnRadioTower(staticObj: any, init: boolean, baseObj?: typing.IBase, side?: number): Promise<void> {
    ...
    const curBaseName = baseObj ? baseObj._id : "";
...
export async function destroyUnit(unitName: string, type: string): Promise<void> {
    ...
    console.log("Destroy Unit: ", unitName);
...
export async function healBase(baseName: string, curPlayerUnit: any, init: boolean): Promise<boolean> {
    ...
    const baseUnit = await ddcsControllers.campaignAirfieldActionRead({_id: baseName});
    // console.log("healBase: ", baseName, baseUnit, curPlayerUnit);
    if (baseUnit.length > 0) {
        ...
        if (curBase.side !== 0 && curBase.side !== curPlayerUnit.coalition) {
            ...
            if (curBase.baseType !== "MOB") {
                await spawnSupportBaseGrp(curBase._id, curPlayerUnit.coalition, false); // return resp

                const shelterUnit = await ddcsControllers.unitActionRead({_id: curBase._id + " Shelter", dead: false});
                    ...
                    curShelterUnit.country =
                        ddcsControllers.countryId.indexOf(
                            _.intersection(
                                ...
                                ddcsControllers.engineCache.campaign.countrySides[(curBase.side || 0)]
                            )[0])
                    ;
                    await ddcsControllers.spawnStaticBaseBuilding(
                        curShelterUnit,
                ...
                const commUnit = await ddcsControllers.unitActionRead({_id: curBase._id + " Comms tower M", dead: false});
                ...
                if (curCommUnit) {
                    ...
                    await ddcsControllers.spawnStaticBaseBuilding({} as IStaticSpawnMin,
                        true, curBase, curPlayerUnit.coalition, "Comms tower M", true);
                }
                // await ddcsControllers.unitActionDelete({_id: curBase._id + " Comms tower M"});
                await spawnSupportBaseGrp(curBase._id, curPlayerUnit.coalition, init);
            }
        }
        return true;
    }

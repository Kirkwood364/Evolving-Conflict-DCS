...
export async function updateLatestCampaign(): Promise<void> {
    ...
    if (campaign) {
        const campSessions = await ddcsControllers.sessionsActionsRead({campaignName: campaign._id});
...
export async function recordFiveMinutesPlayed() {
    ...
    if (latestSession) {
        ...
        await updateSession(latestSession._id);
        console.log("Total Server Players: " + latestSession.playersOnlineArray.length + " PlayerFiveMinCount: ", totalMinsPerSide);
    }
}

...
export async function getAllEWRUnitNames(): Promise <string[]> {
    ...
    const gciUnits = await ddcsController.unitActionRead({dead: false, type: {$in: engineCache.campaign.GCIDetectTypes}});
...
export async function processGCIDetection(incomingObj: any): Promise<void> {
    ...
    if (dedupeDetectedUnitNames.length > 0) {
        ...
        if (detectedUnits.length > 0) {
            ...
            console.log("GCI Detected Units Not Alive");
        }
    }
}
...
export async function gciUpdatePilots(detectedUnits: any, friendlySide: number) {
    ...
    console.log("Auto GCI Turned on for SIDE: ", friendlySide);
    ...
    for (const player of playerArray) {
        ...
        if (curPlayerUnits.length > 0) {
            ...
            for (const unit of detectedUnits) {
                if (unit.unitCategory === 0 || unit.unitCategory === 1) {
                    ...
                    // Enemy aspect, does not simply equal their heading, its their relative heading to the player.
                    // Disabled for now, will display targets heading.

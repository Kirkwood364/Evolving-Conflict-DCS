...
export async function aiDefendBase(): Promise<void> {
    ...
    const mainBaseMOBs = await ddcsController.campaignAirfieldActionRead({baseType: "MOB", _id: { $not: /^~/ }, enabled: true});
        ...
        if (firstUnitEachGroup.length > 0) {
            ...
            for (const unit of firstUnitEachGroup) {
                ...
                if (!!unit.pursuingUnit && new Date().getTime() > new Date(unit.pursueExpiration).getTime()) {
                    ...
                    const newBaseParkingSpot = ddcsController.getRandomLatLonFromBase(base._id, "unitPoly");
    ...
    if (aiGroundUnits.length > 0) {
        for (const unit of aiGroundUnits) {
            ...
            if (!!unit.pursuingUnit && new Date().getTime() > new Date(unit.pursueExpiration).getTime()) {
                ...
                const destBase = await ddcsController.campaignAirfieldActionRead({_id: unit._id.split("|")[3]});

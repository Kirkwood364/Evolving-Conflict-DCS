...
export async function processEventLand(eventObj: any): Promise<void> {
    ...
    if (iUnit.length > 0) {
        ...
        if (_.includes(curUnitName, "LOGISTICS|")) {
            ...
            const bases = await ddcsControllers.campaignAirfieldActionRead({_id: bName});
            if (bases.length > 0) {
        ...
        if (iPlayer) {
            // console.log("landing: ", iPlayer.name);
            ...
            // console.log("land: ", friendlyBases, iUnit[0].lonLatLoc, curUnitSide);
            if (friendlyBases.length > 0) {
                ...
                if (_.includes(engineCache.campaign.bubbleMap[engineCache.campaign.currentCampaignBubble].
                    freeAirframeBases, curUnit.groupName.split(" @")[0])) {
                ...
                console.log("Landing: ", iPlayer.name, " -> +", warbondsToAdd);
        ...
        console.log("LAND: Can't Find Initiator Id: ", eventObj.data.initiator);
    }
}
